...
function WaveDivider({ from, to, flip = false }: { from: string; to: string; flip?: boolean }) {
  return (
    <div style={{ background: from, lineHeight: 0, transform: flip ? 'scaleX(-1)' : undefined }}>
      <svg viewBox="0 0 1440 90" width="100%" height="70" preserveAspectRatio="none">
        <defs>
          <linearGradient id={`wave-${from}-${to}`.replace(/[^a-zA-Z0-9-]/g, '')} x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor="#0EA5E9" />
            <stop offset="100%" stopColor="#7C3AED" />
          </linearGradient>
        </defs>
        <path d="M0,32 C240,80 480,0 720,28 C960,56 1200,84 1440,24 L1440,90 L0,90 Z" fill={to} />
        <path d="M0,20 C240,64 480,-8 720,18 C960,46 1200,70 1440,12" fill="none" stroke={`url(#wave-${from}-${to})`.replace(/[^a-zA-Z0-9#()-]/g, m => m)} strokeWidth="2.5" opacity="0.55" />
      </svg>
    </div>
  )
}
...
const features: { icon: IconName; title: string; body: string; tint: string }[] = [
  ...
  { icon: 'shield', title: 'Verified Experts', body: 'Every expert passes a 2-hour live simulation test. Only top 15% accepted.', tint: '#A78BFA' },
  ...
  { icon: 'cpu', title: 'Smart Matching', body: 'AI matches your project to the highest-rated available expert by skill.', tint: '#A78BFA' },
  { icon: 'globe', title: 'Global Platform', body: 'Multi-currency, multi-timezone. Submit anywhere, get results worldwide.', tint: '#38BDF8' },
  { icon: 'bar-chart', title: 'Full Transparency', body: 'Track every stage: Received → Assigned → QC → Completed.', tint: '#A78BFA' },
  { icon: 'lock', title: 'NDA Support', body: 'Enable confidential mode. Expert must sign NDA before accessing files.', tint: '#38BDF8' },
  { icon: 'credit-card', title: 'Flexible Payments', body: 'Pay 50% advance, 50% on delivery. Wallet system for repeat clients.', tint: '#A78BFA' },
  ...
  return (
    <>
      <style>{`
        ...
        .fcard { background:rgba(255,255,255,.03); border:1px solid rgba(255,255,255,.08); border-radius:18px; padding:30px 26px; transition:all .35s cubic-bezier(.22,.61,.36,1); }
        .fcard:hover { background:linear-gradient(160deg, rgba(14,165,233,.10), rgba(124,58,237,.10)); border-color:rgba(148,163,255,.35); transform:translateY(-5px); }
        .scard { background:#fff; border:1px solid #E2E8F0; border-radius:20px; padding:30px; transition:all .35s cubic-bezier(.22,.61,.36,1); }
        .scard:hover { box-shadow:0 18px 50px rgba(76,29,149,.10); transform:translateY(-5px); border-color:#DDD6FE; }
        .stcard { background:#fff; border-radius:20px; padding:34px 24px; text-align:center; border:1px solid #F1F5F9; transition:all .35s cubic-bezier(.22,.61,.36,1); }
        .stcard:hover { transform:translateY(-5px); box-shadow:0 18px 44px rgba(11,28,61,.09); }
        .tcard { background:#fff; border-radius:20px; padding:28px; border:1px solid #EEF2FF; transition:all .35s cubic-bezier(.22,.61,.36,1); position:relative; overflow:hidden; }
        .tcard:hover { transform:translateY(-5px); box-shadow:0 20px 48px rgba(76,29,149,.12); border-color:#DDD6FE; }
        .tcard::before { content:""; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg,#0EA5E9,#7C3AED); opacity:.0; transition:opacity .35s; }
        .tcard:hover::before { opacity:1; }
        .btncta { display:inline-flex; align-items:center; gap:8px; background:linear-gradient(120deg,#0EA5E9,#7C3AED); color:#fff; font-family:'Syne',sans-serif; font-weight:700; font-size:15.5px; padding:15px 30px; border-radius:12px; border:none; cursor:pointer; transition:all .25s cubic-bezier(.22,.61,.36,1); text-decoration:none; background-size:160% 160%; background-position:0% 50%; }
        .btncta:hover { background-position:100% 50%; transform:translateY(-2px); box-shadow:0 12px 30px rgba(124,58,237,.35); color:#fff; }
        .btngl { display:inline-flex; align-items:center; gap:8px; background:transparent; color:rgba(255,255,255,.88); font-family:'DM Sans',sans-serif; font-weight:500; font-size:15px; padding:14px 25px; border-radius:12px; border:1px solid rgba(255,255,255,.2); cursor:pointer; transition:all .25s cubic-bezier(.22,.61,.36,1); text-decoration:none; }
        .btngl:hover { background:rgba(255,255,255,.08); border-color:rgba(167,139,250,.5); color:#fff; }
        ...
        .lcard { background:#fff; border-radius:20px; padding:26px; border:1px solid #F1F5F9; text-align:center; transition:all .35s cubic-bezier(.22,.61,.36,1); }
        .lcard:hover { box-shadow:0 14px 40px rgba(76,29,149,.10); transform:translateY(-4px); }
        .blob { position:absolute; border-radius:50%; filter:blur(80px); pointer-events:none; }
        .iconwrap { width:46px; height:46px; border-radius:12px; display:flex; align-items:center; justify-content:center; margin-bottom:18px; }
      ...
      <div className="lp">
        ...
        <nav className={`${navScrolled ? 'navglass' : 'navsolid'}`} style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, transition: 'all .3s' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none' }}>
              <GSHLogo size={46} />
              <div>
                <div className="dp" style={{ color: '#fff', fontWeight: 700, fontSize: 16, lineHeight: 1.2, letterSpacing: '.02em' }}>Global Simulation Hub</div>
                <div style={{ color: '#A78BFA', fontSize: 9.5, fontWeight: 600, letterSpacing: '.18em', textTransform: 'uppercase', marginTop: 1 }}>Engineering · Simulation · Delivery</div>
              </div>
            </Link>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Link to="/apply-expert" style={{ color: 'rgba(255,255,255,.7)', fontSize: 13.5, fontWeight: 500, padding: '8px 14px', borderRadius: 8, textDecoration: 'none' }}>Become Expert</Link>
              <Link to="/login" className="btngl" style={{ fontSize: 13.5, padding: '9px 17px' }}>Login</Link>
              <Link to="/submit" className="btncta" style={{ fontSize: 13.5, padding: '10px 20px' }}>
                Submit Project
        ...
        <section style={{ minHeight: '100vh', background: '#0B1C3D', position: 'relative', display: 'flex', alignItems: 'center', paddingTop: 96, paddingBottom: 64, overflow: 'hidden' }} className="hgrid">
          ...
          <div className="blob drift" style={{ width: 460, height: 460, background: 'rgba(124,58,237,.14)', bottom: '10%', right: '6%', animationDelay: '3s' }} />
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }} className="grid-2">
            <div>
              ...
              <h1 className="dp au1" style={{ fontSize: 'clamp(38px, 4.6vw, 64px)', fontWeight: 800, lineHeight: 1.06, color: '#fff', margin: '0 0 24px' }}>
                Simulation<br />
                <span style={{ background: 'linear-gradient(120deg,#38BDF8,#A78BFA)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Delivered Fast</span><br />
                Globally.
              </h1>
              <p className="au2" style={{ color: '#A3AFC4', fontSize: 18, lineHeight: 1.7, maxWidth: 470, margin: '0 0 36px' }}>
              ...
              <div className="au3" style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 40 }}>
                <Link to="/submit" className="btncta" style={{ fontSize: 15.5, padding: '16px 32px' }}>
                  Submit a Project
                  <Icon name="arrow-right" size={16} />
                </Link>
                <Link to="/apply-expert" className="btngl" style={{ fontSize: 15.5, padding: '16px 28px' }}>Join as Expert</Link>
              </div>
              <div className="au4" style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                {['No account needed', 'Secure delivery', 'Expert-verified'].map((t) => (
                  <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ color: '#7C8CA6', fontSize: 13.5 }}>{t}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="au" style={{ position: 'relative', padding: '20px 0' }}>
              <div style={{ background: 'rgba(255,255,255,.04)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 24, padding: 28, boxShadow: '0 32px 80px rgba(0,0,0,.4)', animation: 'floatY 6s ease-in-out infinite' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, background: 'linear-gradient(135deg,#0EA5E9,#7C3AED)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 14, fontFamily: 'Syne,sans-serif' }}>M</div>
                    <div>
                      <p style={{ color: '#fff', fontSize: 14.5, fontWeight: 600, margin: 0, lineHeight: 1.3 }}>DC Motor Speed Control</p>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ height: 8, background: 'rgba(255,255,255,.08)', borderRadius: 99, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: '73%', background: 'linear-gradient(90deg,#0EA5E9,#A78BFA)', borderRadius: 99, transition: 'width 1.2s ease' }} />
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'rgba(255,255,255,.04)', borderRadius: 14, border: '1px solid rgba(255,255,255,.06)', marginBottom: 16 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#7C3AED,#0EA5E9)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 12.5, flexShrink: 0 }}>AK</div>
                  <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, color: '#7C8CA6' }}>
              ...
              <div style={{ position: 'absolute', top: -8, right: -8, background: '#10B981', color: '#fff', fontSize: 11.5, fontWeight: 700, padding: '6px 14px 6px 12px', borderRadius: 999, boxShadow: '0 4px 16px rgba(16,185,129,.4)', animation: 'floatY 4s .5s ease-in-out infinite', display: 'flex', alignItems: 'center', gap: 5 }}>
                <Icon name="check" size={12} color="#fff" strokeWidth={3.2} />QC Passed
              </div>
              <div style={{ position: 'absolute', bottom: -8, left: -8, background: '#fff', color: '#0B1C3D', fontSize: 11.5, fontWeight: 700, padding: '6px 14px', borderRadius: 999, boxShadow: '0 4px 20px rgba(76,29,149,.18)', display: 'flex', alignItems: 'center', gap: 6, animation: 'floatY 5s 1s ease-in-out infinite' }}>
                <span style={{ width: 7, height: 7, background: 'linear-gradient(135deg,#0EA5E9,#7C3AED)', borderRadius: '50%' }} />1,200+ Projects Delivered
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="mq" style={{ display: 'flex', width: 'max-content' }}>
            {[...tools, ...tools].map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '0 28px' }}>
                <span style={{ color: '#3A4A6B', fontSize: 12.5, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{t}</span>
        ...
        {/* transition: navy -> light, blue/purple gradient wave */}
        <WaveDivider from="#0B1C3D" to="#F8FAFC" />

        {/* STATS */}
        <section ref={statsRef.ref} style={{ padding: '70px 24px 80px', background: '#F8FAFC' }}>
          <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }} className="grid-4">
            {[
              ...
              <div key={label} className="stcard">
                <p className="dp" style={{ fontSize: 46, fontWeight: 800, color: '#0B1C3D', margin: '0 0 6px', lineHeight: 1 }}>{value}{suffix}</p>
                <p style={{ color: '#94A3B8', fontSize: 13.5, margin: 0 }}>{label}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <Reveal>
              <div style={{ textAlign: 'center', marginBottom: 56 }}>
                <span style={{ color: '#7C3AED', fontSize: 11.5, fontWeight: 700, letterSpacing: '.16em', textTransform: 'uppercase', display: 'block', marginBottom: 12 }}>How It Works</span>
                <h2 className="dp" style={{ fontSize: 'clamp(30px,3.4vw,42px)', fontWeight: 800, color: '#0B1C3D', margin: 0 }}>Four steps to results</h2>
              </div>
            </Reveal>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }} className="grid-4">
              {steps.map((s, i) => (
                <Reveal key={i} delay={i * 90}>
                  <div className="scard">
                    <div className="dp" style={{ fontSize: 52, fontWeight: 800, background: 'linear-gradient(135deg,#E0F2FE,#EDE9FE)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1, marginBottom: 16 }}>{s.n}</div>
                    <h3 className="dp" style={{ fontSize: 18, fontWeight: 700, color: '#0B1C3D', margin: '0 0 10px' }}>{s.title}</h3>
                    <p style={{ color: '#64748B', fontSize: 14, lineHeight: 1.65, margin: 0 }}>{s.body}</p>
                    <div style={{ width: 32, height: 3, background: 'linear-gradient(90deg,#0EA5E9,#7C3AED)', borderRadius: 99, marginTop: 20 }} />
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        {/* FEATURES */}
        <section style={{ padding: '90px 24px 96px', background: '#0B1C3D', position: 'relative', overflow: 'hidden' }}>
          <div className="blob" style={{ width: 460, height: 460, background: 'rgba(124,58,237,.10)', top: '-10%', right: '-6%' }} />
          <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative' }}>
            <Reveal>
              <div style={{ textAlign: 'center', marginBottom: 56 }}>
                ...
                <h2 className="dp" style={{ fontSize: 'clamp(30px,3.4vw,42px)', fontWeight: 800, color: '#fff', margin: 0 }}>Why engineers choose GSH</h2>
              </div>
            </Reveal>
              ...
              {features.map(({ icon, title, body, tint }, i) => (
                <Reveal key={title} delay={(i % 3) * 90}>
                  <div className="fcard">
                    ...
                    <h3 className="dp" style={{ color: '#fff', fontWeight: 700, fontSize: 16.5, margin: '0 0 8px' }}>{title}</h3>
        ...
        {/* TESTIMONIALS */}
        <section style={{ padding: '90px 24px 96px', background: '#F8FAFC' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <Reveal>
              <div style={{ textAlign: 'center', marginBottom: 48 }}>
                ...
                <h2 className="dp" style={{ fontSize: 'clamp(30px,3.4vw,42px)', fontWeight: 800, color: '#0B1C3D', margin: '0 0 12px' }}>Trusted by engineers worldwide</h2>
                <p style={{ color: '#94A3B8', fontSize: 15, maxWidth: 500, margin: '0 auto' }}>Real results from students and professionals who needed simulations done right.</p>
              </div>
            </Reveal>

            <Reveal>
              <div style={{ background: '#0B1C3D', borderRadius: 24, padding: '44px 48px', marginBottom: 28, position: 'relative', overflow: 'hidden' }}>
                <div className="blob" style={{ width: 260, height: 260, background: 'rgba(124,58,237,.18)', top: '-30%', left: '-6%' }} />
                ...
                <div style={{ position: 'relative', maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
                  <Icon name="quote" size={30} color="#A78BFA" />
                  <p className="dp" style={{ color: '#fff', fontSize: 22, lineHeight: 1.55, fontWeight: 600, margin: '20px 0 24px' }}>
                    "{featured.quote}"
                  </p>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                    <Avatar photo={featured.photo} initials={featured.initials} size={42} gradient="linear-gradient(135deg,#0EA5E9,#7C3AED)" />
                    <div style={{ textAlign: 'left' }}>
                      <p style={{ color: '#fff', fontWeight: 600, fontSize: 14, margin: 0 }}>{featured.name}</p>
                      <p style={{ color: '#7C8CA6', fontSize: 12.5, margin: 0 }}>{featured.role}</p>
                    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 18 }} className="grid-3">
              {testimonials.slice(1).map((t, i) => (
                <Reveal key={t.name} delay={i * 100}>
                  <div className="tcard">
                    <Icon name="quote" size={20} color="#C4B5FD" />
                    <p style={{ color: '#334155', fontSize: 14.5, lineHeight: 1.7, margin: '16px 0 22px' }}>{t.quote}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <Avatar photo={t.photo} initials={t.initials} size={38} gradient={i % 2 === 0 ? 'linear-gradient(135deg,#0EA5E9,#38BDF8)' : 'linear-gradient(135deg,#7C3AED,#A78BFA)'} />
                      <div style={{ flex: 1 }}>
          ...
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            <Reveal>
              <div style={{ textAlign: 'center', marginBottom: 52 }}>
                <span style={{ color: '#7C3AED', fontSize: 11.5, fontWeight: 700, letterSpacing: '.16em', textTransform: 'uppercase', display: 'block', marginBottom: 12 }}>Expert Tiers</span>
                <h2 className="dp" style={{ fontSize: 'clamp(30px,3.4vw,42px)', fontWeight: 800, color: '#0B1C3D', margin: '0 0 12px' }}>Ranked by performance</h2>
                <p style={{ color: '#94A3B8', fontSize: 15, maxWidth: 480, margin: '0 auto' }}>Every expert advances based on completed projects, ratings, and success rate.</p>
              </div>
            </Reveal>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }} className="grid-4">
              {levels.map((l, i) => (
                <Reveal key={l.label} delay={i * 90}>
                  <div className="lcard">
                    <div style={{ width: 50, height: 50, borderRadius: '50%', background: l.color + '16', margin: '0 auto 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                      <StarRow count={l.stars} size={11} color={l.color} />
                    </div>
                    <p className="dp" style={{ fontWeight: 700, color: '#0B1C3D', margin: '0 0 4px', fontSize: 15.5 }}>{l.label}</p>
                    <p style={{ color: '#94A3B8', fontSize: 12.5, margin: '0 0 14px' }}>{l.projects}</p>
        ...
        {/* CTA */}
        <section style={{ padding: '96px 24px', background: 'linear-gradient(135deg,#0B1C3D 0%,#1A3A7A 45%,#5B21B6 100%)', position: 'relative', overflow: 'hidden', textAlign: 'center' }} className="hgrid">
          <div className="blob drift" style={{ width: 500, height: 300, background: 'rgba(167,139,250,.14)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
          <Reveal>
            <div style={{ position: 'relative', maxWidth: 640, margin: '0 auto' }}>
              ...
              <h2 className="dp" style={{ fontSize: 'clamp(32px,4vw,48px)', fontWeight: 800, color: '#fff', margin: '0 0 20px', lineHeight: 1.1 }}>
                Ready to get your<br />
                <span style={{ background: 'linear-gradient(120deg,#38BDF8,#C4B5FD)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>simulation done?</span>
              </h2>
              <p style={{ color: '#93A2C4', fontSize: 15.5, margin: '0 0 40px', lineHeight: 1.7 }}>No account needed. Describe your project and we'll set everything up automatically.</p>
              <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
                <Link to="/submit" className="btncta" style={{ fontSize: 15.5, padding: '17px 36px' }}>
                  Submit a Project
                  <Icon name="arrow-right" size={16} />
                </Link>
                <Link to="/apply-expert" className="btngl" style={{ fontSize: 15.5, padding: '17px 32px' }}>Become an Expert</Link>
              </div>
            </div>
          </Reveal>
        </section>
        ...
        <footer style={{ background: '#040C1F', padding: '56px 24px 28px', borderTop: '1px solid rgba(255,255,255,.04)' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 48, marginBottom: 48 }} className="grid-2">
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  <GSHLogo size={40} />
                  <div>
                    <p className="dp" style={{ color: '#fff', fontWeight: 700, fontSize: 14.5, margin: 0 }}>Global Simulation Hub</p>
                    <p style={{ color: '#3A4A6B', fontSize: 11.5, margin: 0 }}>Engineering excellence, delivered.</p>
                  </div>
                </div>
                <p style={{ color: '#3A4A6B', fontSize: 13.5, lineHeight: 1.7, margin: 0 }}>Connecting the world's best simulation engineers with clients who need results — fast, verified, and secure.</p>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32 }} className="grid-3">
                {[
                  ...
                  <div key={title}>
                    <p style={{ color: '#fff', fontWeight: 600, fontSize: 13.5, margin: '0 0 14px' }}>{title}</p>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,.04)', paddingTop: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <p style={{ color: '#243352', fontSize: 12.5, margin: 0 }}>© {new Date().getFullYear()} Global Simulation Hub. All rights reserved.</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                <span style={{ width: 7, height: 7, background: '#4ADE80', borderRadius: '50%' }} />
                <span style={{ color: '#243352', fontSize: 12.5 }}>All systems operational</span>
              </div>
            </div>
          </div>
        </footer>
