  ...
  return (
    <div className="ec-page">
      <style>{`
        ...
        .ec-page { font-family:'DM Sans',sans-serif; font-size:14px; }
        .ec-title { font-family:'Syne',sans-serif; font-size:24px; font-weight:800; color:#0F172A; margin:0; display:flex; align-items:center; gap:10px; }
        .ec-body { display:flex; gap:16px; height:560px; }
        .ec-sidebar { width:260px; flex-shrink:0; background:#fff; border-radius:20px; border:1px solid #F1F5F9; overflow:hidden; display:flex; flex-direction:column; }
        .ec-chatarea { flex:1; background:#fff; border-radius:20px; border:1px solid #F1F5F9; display:flex; flex-direction:column; overflow:hidden; min-width:0; }
        .ec-mobile-back { display:none; }

        @media (max-width: 760px) {
          .ec-page { font-size:13px; }
          .ec-title { font-size:19px; }
          .ec-body { height: calc(100vh - 210px); min-height:420px; gap:0; }
          .ec-sidebar { width:100%; border-radius:16px; }
          .ec-chatarea { border-radius:16px; }
          .ec-mobile-back { display:flex; }
          .ec-sidebar.ec-hide-mobile { display:none; }
          .ec-chatarea.ec-hide-mobile { display:none; }
        }
      `}</style>
      ...
      <div className="fu" style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: '#94A3B8', fontSize: 13, margin: '4px 0 0' }}>Chat with GSH admin about your assigned projects.</p>
      </div>

      <div className="ec-body">
        {/* Sidebar — rooms */}
        <div className={`ec-sidebar ${mobileView === 'chat' ? 'ec-hide-mobile' : ''}`}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {roomsLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: '40px 16px', color: '#94A3B8' }}>
                ...
                <p style={{ fontSize: 12, lineHeight: 1.5, margin: 0 }}>Admin will reach out about your assigned projects.</p>
              </div>
            ) : rooms.map(r => {
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <p style={{ fontFamily: 'Syne,sans-serif', fontWeight: 700, fontSize: 13, color: '#0F172A', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {r.other_user?.name || 'GSH Admin'}
                        </p>
                        <p style={{ fontSize: 11, color: '#94A3B8', margin: 0 }}>
                          {r.project ? `Project #${r.project}` : 'GSH Admin Team'}
                        </p>
                      </div>
                    </div>
                    {r.unread_count > 0 && (
        ...
        {/* Main chat area */}
        <div className={`ec-chatarea ${mobileView === 'list' ? 'ec-hide-mobile' : ''}`}>
          {!selectedRoom ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#94A3B8', padding: 32, textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, margin: 0, maxWidth: 260, lineHeight: 1.6 }}>Your messages with GSH admin will appear here.</p>
            </div>
          ) : (
            <>
              {/* Chat header */}
              <div style={{ padding: '14px 20px', borderBottom: '1px solid #F8FAFC', display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontFamily: 'Syne,sans-serif', fontWeight: 700, fontSize: 14, color: '#0F172A', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {selectedRoom.other_user?.name || 'GSH Admin'}
                  </p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#10B981', display: 'inline-block' }} />
                    <span style={{ fontSize: 11, color: '#10B981', fontWeight: 600 }}>Online</span>
                    {selectedRoom.project && (
                      <span style={{ marginLeft: 6, fontSize: 11, color: '#94A3B8' }}>· Project #{selectedRoom.project}</span>
                    )}
                  </div>
                </div>
              </div>
