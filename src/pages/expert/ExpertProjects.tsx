...
import { Search, Inbox, Undo2, Clock, ClipboardList } from 'lucide-react'
...
const STATUSES = ['','assigned','in_progress','qc','revision','completed']
...
export default function ExpertProjects() {
  ...
  // Expert should only ever see projects assigned to them — the backend's
  // list endpoint should already scope this to request.user for expert role,
  // same pattern as ClientProjects scoping to the client automatically.
  const { data, isLoading } = useQuery({
    queryKey: ['expert-projects-list', statusFilter],
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
      <style>{`
        ...
        .ep-search-input { transition: border-color .15s ease, box-shadow .15s ease; }
        .ep-search-input:focus { border-color: #0EA5E9; box-shadow: 0 0 0 3px rgba(14,165,233,.12); }
        .ep-select { transition: border-color .15s ease; }
        .ep-project-card { transition: box-shadow 0.2s ease, transform 0.2s ease; }
        .ep-job-board-btn { transition: transform 0.15s ease, opacity 0.15s ease, box-shadow 0.2s ease; }
        .ep-job-board-btn:hover { transform: translateY(-2px); opacity: 0.95; box-shadow: 0 8px 18px rgba(14,165,233,.25); }
        .ep-job-board-btn:active { transform: translateY(0); }
      `}</style>
      ...
      <div className="fu" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12, marginBottom:24 }}>
        <div>
          ...
          <p style={{ color:'#94A3B8', fontSize:13, margin:'4px 0 0' }}>{data?.count || 0} assigned projects</p>
        </div>
        <Link to="/expert/job-board" className="ep-job-board-btn" style={{ display:'inline-flex', alignItems:'center', gap:7, background:'#0EA5E9', color:'#fff', fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:13, padding:'10px 20px', borderRadius:12, textDecoration:'none' }}>
          <ClipboardList size={15} strokeWidth={2.4} /> Browse Job Board
        </Link>
      </div>
      ...
      <div className="fu" style={{ display:'flex', gap:10, flexWrap:'wrap', marginBottom:20 }}>
        ...
        <select className="ep-select" value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ padding:'10px 14px', border:'1.5px solid #E2E8F0', borderRadius:10, fontSize:13, background:'#fff', cursor:'pointer', fontFamily:'DM Sans,sans-serif', outline:'none' }}>
      ...
      {isLoading ? (
        ...
        <div className="fu" style={{ background:'#fff', borderRadius:20, border:'1px solid #F1F5F9', padding:'64px 24px', textAlign:'center' }}>
          ...
          <p style={{ fontFamily:'Syne,sans-serif', fontSize:18, fontWeight:700, color:'#0F172A', margin:'0 0 8px' }}>{search || statusFilter ? 'No matching projects' : 'No projects assigned yet'}</p>
          <p style={{ fontSize:13, color:'#94A3B8', margin:'0 0 20px' }}>{search || statusFilter ? 'Try a different filter.' : 'Browse the job board to pick up your first project.'}</p>
          {!search && !statusFilter && (
            <Link to="/expert/job-board" className="ep-job-board-btn" style={{ display:'inline-flex', alignItems:'center', gap:7, background:'#0EA5E9', color:'#fff', fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:13, padding:'11px 22px', borderRadius:12, textDecoration:'none' }}>Browse Job Board →</Link>
          )}
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {projects.map((p, i) => {
            ...
            return (
              <div key={p.id} className="fu ep-project-card" onClick={() => navigate(`/expert/projects/${p.id}`)}
                ...
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:12, marginBottom:14 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6, flexWrap:'wrap' }}>
                      ...
                      {p.status === 'qc' && (
                        <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:10, background:'#F5F3FF', color:'#7C3AED', padding:'2px 8px', borderRadius:999, fontWeight:700 }}>
                          <Clock size={10} strokeWidth={2.6} /> AWAITING QC
                        </span>
                      )}
                    </div>
                    <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center' }}>
                      ...
                      {p.client_name && (
                        <span style={{ fontSize:12, color:'#64748B' }}>Client: {p.client_name}</span>
                      )}
                    </div>
                  </div>
