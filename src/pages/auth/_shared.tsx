...
.abtn { width:100%; display:flex; align-items:center; justify-content:center; gap:8px; background:linear-gradient(120deg,#0EA5E9,#7C3AED); background-size:160% 160%; background-position:0% 50%; color:#fff; font-family:'Syne',sans-serif; font-weight:700; font-size:15px; padding:14px; border-radius:12px; border:none; cursor:pointer; transition:all .25s cubic-bezier(.22,.61,.36,1); letter-spacing:.01em; }
.abtn:hover:not(:disabled) { background-position:100% 50%; box-shadow:0 10px 26px rgba(124,58,237,.32); transform:translateY(-1px); }
...
export function AuthCard({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ minHeight: '100vh', background: '#0B1C3D', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden' }} className="hgrid ap">
      ...
      <div className="ablob" style={{ width: 300, height: 300, background: 'rgba(124,58,237,.10)', bottom: '10%', right: '-5%', animationDelay: '3s' }} />
